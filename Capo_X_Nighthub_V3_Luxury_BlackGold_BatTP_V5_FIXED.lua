...
local function CAPO_emergencyToast(msg)
    pcall(function()
        local sg = Instance.new("ScreenGui")
        sg.Name = "CapoBootToast"
        ...
        s.Color = Color3.fromRGB(180, 180, 190)
        s.Thickness = 1.4
        local t = Instance.new("TextLabel", f)
        t.Size = UDim2.fromScale(1, 1)
        t.BackgroundTransparency = 1
        t.Text = tostring(msg or "Capo X Nighthub V3 loading...")
...
M.UI_COLORS = {
    { name = "Black",  accent = Color3.fromRGB(255, 210, 80) }, -- Luxury Gold
}
M.uiColorName = "Black"
...
function M.accentColor()
    return Color3.fromRGB(255, 210, 80) -- locked Luxury Gold
end

function M.accentDim()
    return Color3.fromRGB(180, 130, 30) -- locked gold dim
end
...
function M.bgImageIdForColor(name)
    name = name or M.uiColorName or "Black"
    ...
    task.spawn(function()
        for _ = 1, 64 do
            ...
            elseif ok and res then
                ...
                if (M.uiColorName or "Black") == want then
...
M.TITLE_VS_COLOR = Color3.fromRGB(255, 215, 90)

function M.headerTitleRich()
    return '<font color="rgb(255,215,90)">Capo X</font> <font color="rgb(255,255,255)">Nighthub V3</font>'
end
...
function M.bgTintColor(a)
    a = a or M.accentColor()
    if (M.uiColorName or "Black") == "Ocean" then return Color3.fromRGB(255, 255, 255) end
...
function M.applyBgRecolor(img)
    ...
    local name = M.uiColorName or "Black"
...
function M.applyBgWash(img, corner)
    ...
    if not w then
        ...
        c.Name = "WashCorner"
        c.CornerRadius = corner or UDim.new(0, M.MENU_CORNER_R or 4)
        c.Parent = w
    end
    w.BackgroundColor3 = M.accentColor()
    w.BackgroundTransparency = 0.9
...
function M._skinClearOutfit(char)
    ...
    for _, obj in ipairs(char:GetChildren()) do
        if obj:IsA("Shirt") or obj:IsA("Pants") or obj:IsA("Accessory") or obj:IsA("Hat")
            or obj.Name == "AuFfitAccessory" or obj.Name == "CapoSkinPackAcc" then
...
function M._skinApplyPackConfig(char, config)
    ...
    if config.accessory and head then
        local old = char:FindFirstChild("AuFfitAccessory") or char:FindFirstChild("CapoSkinPackAcc")
...
function M.clearCapoHorns(char)
end

function M.buildCapoHorns(char, head)
end

function M.equipCapoHorns(char)
end

function M.setCapoHorns(on)
    M.vynxHornsEnabled = false
    if M.setCapoHornsVisual then pcall(function() M.setCapoHornsVisual(false) end) end
end
...
function M.applyCharterToChar(char)
    ...
    if M.removeAccEnabled then
        for _, child in ipairs(char:GetChildren()) do
            if child:IsA("Accessory") or child:IsA("Hat") then
                if not (child:GetAttribute("VynxSkin") or child:GetAttribute("CapoHorn") or (child.Name and (child.Name:find("Capo X Nighthub V3") or child.Name:find("CapoHorn")))) then
                    pcall(function() child:Destroy() end)
                end
            end
        end
    ...
    if false and M.vynxHornsEnabled then
        pcall(function() M.equipCapoHorns(char) end)
    end
...
function M.stripVynxClothing(char)
    ...
    for _, c in ipairs(char:GetChildren()) do
        if c:IsA("Shirt") or c:IsA("Pants")
            ...
            if not c:GetAttribute("CapoChip") then
...
function M.clearCapoTorsoChip(char)
    if not char then return end
    for _, part in ipairs(char:GetDescendants()) do
        if part.Name == "CapoTorsoChipBB" or part.Name == "CapoTorsoChipSG" then
            pcall(function() part:Destroy() end)
        end
    end
end

function M.attachCapoPantsTag(char)
    -- Capo X Nighthub V3 pants/shirt text tags removed
    if not char then return end
    local legNames = {
    ...
    for _, name in ipairs(legNames) do
        ...
        if leg then
            for _, c in ipairs(leg:GetChildren()) do
                if c.Name == "CapoPantsTag" or c:GetAttribute("CapoChip") then
                    pcall(function() c:Destroy() end)
                end
            end
        end
...
function M.attachCapoBellyTag(char)
    -- Capo X Nighthub V3 belly/torso text tags removed
    if not char then return end
    local torso = char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso")
    if not torso then return end
    for _, c in ipairs(torso:GetChildren()) do
        if c.Name == "CapoBellyTag" or c.Name == "CapoTorsoChipBB" or c.Name == "CapoTorsoChipSG" then
            pcall(function() c:Destroy() end)
        end
    end
end

function M.attachCapoHat(char)
    if not char then return end
    local head = char:FindFirstChild("Head")
    local hum = char:FindFirstChildOfClass("Humanoid")
    if not head then return end
    for _, c in ipairs(char:GetChildren()) do
        if c.Name == "CapoHat" or c.Name == "CapoHatBrim" or (c:IsA("Accessory") and c:GetAttribute("CapoChip")) then
            pcall(function() c:Destroy() end)
        end
    end
    ...
    acc.Name = "CapoHat"
    acc:SetAttribute("CapoChip", true)
...
function M.applyVynxBlackSkin(char)
    ...
    for _, p in ipairs(char:GetDescendants()) do
        ...
        elseif p:IsA("SpecialMesh") and p.Parent and p.Parent.Name ~= "CapoHat" then
            pcall(function()
                p.TextureId = ""
            end)
    ...
    -- Capo X Nighthub V3 text tags disabled
    pcall(function() M.attachCapoBellyTag(char) end) -- cleans only
    pcall(function() M.attachCapoPantsTag(char) end) -- cleans only
    M.attachCapoHat(char)
    ...
    M._vynxBlackChildConn = char.ChildAdded:Connect(function(c)
        if not M.vynxBlackSkinEnabled then return end
        if c:GetAttribute("CapoChip") then return end
        if c.Name == "CapoHat" or c.Name == "CapoHatBrim" or c:GetAttribute("CapoChip") then return end
...
M._antiBatConn = nil
M._holdJumpConn = nil
M._velocityAntiDropActive = false
M._velocitySpoofed = Vector3.zero
M._velocityMtOldIdx = nil
M._velocityMtOldNewIdx = nil
...
local _CAPO_STRETCHREZ_STEP = "CapoStretchRez"

function M.enableStretchRez()
    M.stretchEnabled = true
    pcall(function() RunService:UnbindFromRenderStep(_CAPO_STRETCHREZ_STEP) end)
    pcall(function()
        RunService:BindToRenderStep(_CAPO_STRETCHREZ_STEP, Enum.RenderPriority.Last.Value - 1, function()
...
function M.updateRagdollTimer(duration, labelText)
    ...
    local accent = Color3.fromRGB(255, 210, 80)       -- gold
    local accentDim = Color3.fromRGB(180, 130, 30)
    local bg = Color3.fromRGB(6, 6, 8)              -- near-black
    local bgCard = Color3.fromRGB(8, 8, 10)
    ...
    track.BackgroundColor3 = Color3.fromRGB(18, 18, 20)
...
function M.applyAnimPack(packName)
    ...
    applyingAnim = false
    if not ok then
        warn("[Capo X Nighthub V3] applyAnimPack error: " .. tostring(result))
        return false
    end
    return result == true
end
...
function M._makeESPTracers()
    ...
    local WHITE = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
    ...
    for _, plr in ipairs(currentPlayers) do
        ...
        if alive then
            ...
            if not hl or not hl.Parent or hl.Parent ~= char then
                ...
                hl.FillColor = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
                hl.FillTransparency = 0.72
                hl.OutlineColor = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
            ...
            if not bb or not bb.Parent then
                ...
                stroke.Name = "EspRing"
                stroke.Color = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
...
function M.setupHeadIndicator(char)
    ...
    discordLbl.Text = "discord.gg/3gqtyYgB7"
    ...
    do
        -- Luxury gold gradient (instead of ocean blue)
        local goldDark = Color3.fromRGB(180, 120, 20)
        local goldMid  = Color3.fromRGB(255, 200, 70)
        local goldLite = Color3.fromRGB(255, 240, 180)
        local g = Instance.new("UIGradient")
        g.Name = "DiscordGoldGrad"
        g.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, goldDark),
            ColorSequenceKeypoint.new(0.45, goldMid),
            ColorSequenceKeypoint.new(1, goldLite),
        })
        g.Rotation = 0
        g.Parent = discordLbl
    end
    ...
    divLine.BackgroundColor3 = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
    ...
    speedLbl.TextColor3 = Color3.fromRGB(255, 245, 220)
    ...
    ragdollLbl.Text = ""
    ragdollLbl.TextColor3 = Color3.fromRGB(255, 200, 70)
...
function M.updateHeadTheme()
    ...
    if M.headIndicator.discord then
        ...
        M.headIndicator.discord.Text = "discord.gg/3gqtyYgB7"
    ...
    if M.headIndicator.speed then
        M.headIndicator.speed.TextColor3 = Color3.fromRGB(255, 245, 220)
        M.headIndicator.speed.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
    end
    if M.headIndicator.ragdollTimer then
        M.headIndicator.ragdollTimer.TextColor3 = Color3.fromRGB(255, 200, 70)
    ...
    if M.headIndicator.divider and M.headIndicator.divider.Parent then
        ...
        M.headIndicator.divider.BackgroundColor3 =
            ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
    end
end
...
function M.buildNewStatusUI()
    ...
    do
        ...
        st.Name = "GoldOutline"
        st.Color = Color3.fromRGB(255, 200, 70)
        st.Thickness = 2.8
        st.Transparency = 0
    ...
    -- Solid black steal bar body (no blue anime image)
    local stealBg = Instance.new("ImageLabel")
    stealBg.Name = "StealBarBg"
    stealBg.Size = UDim2.new(1, 0, 1, 0)
    stealBg.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    stealBg.BackgroundTransparency = 0
    stealBg.BorderSizePixel = 0
    stealBg.Image = ""
    stealBg.ImageTransparency = 1
    stealBg.ZIndex = 1
    stealBg.Parent = frame
    Instance.new("UICorner", stealBg).CornerRadius = UDim.new(0, 4)
    ...
    veil.ZIndex = 2
    veil.Parent = frame
    Instance.new("UICorner", veil).CornerRadius = UDim.new(0, 4)

    -- CN logo left (large, own blue outline; V blue + H white)
    ...
    logoWrap.BackgroundColor3 = Color3.fromRGB(4, 4, 6)
    logoWrap.BackgroundTransparency = 0.25
    logoWrap.BorderSizePixel = 0
    logoWrap.ZIndex = 6
    Instance.new("UICorner", logoWrap).CornerRadius = UDim.new(0, 4)
    do
        local ls = Instance.new("UIStroke", logoWrap)
        ls.Color = Color3.fromRGB(255, 200, 70)
        ls.Thickness = 2.2
        ls.Transparency = 0
    end
    ...
    logoV.Text = "C"
    logoV.TextColor3 = Color3.fromRGB(255, 215, 90)
    ...
    sub.Text = "Capo X Nighthub V3"
    sub.TextColor3 = Color3.fromRGB(230, 195, 120)
    ...
    pctLbl.Text = "0%"
    pctLbl.TextColor3 = Color3.fromRGB(255, 245, 220)
    pctLbl.Font = Enum.Font.GothamBlack
    pctLbl.TextSize = 11
    pctLbl.ZIndex = 6
    Instance.new("UICorner", pctLbl).CornerRadius = UDim.new(0, 4)
    ...
    do
        ...
        tst.Color = Color3.fromRGB(200, 200, 210)
        tst.Thickness = 1
    ...
    border.BackgroundColor3 = Color3.fromRGB(40, 40, 45)
    ...
    do
        ...
        M._stealBarStatsConn = RunService.Heartbeat:Connect(function()
            ...
            if M.statusInfoLbl then
                M.statusInfoLbl.Text = "FPS:" .. tostring(fps) .. "  Capo X Nighthub V3  PING: " .. tostring(pingMs) .. "ms"
            end
...
function M.buildStatusUI()
    ...
    local barH = 46
    local RED = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
    local WHITE = Color3.fromRGB(255, 255, 255)
...
function M.updateStealProgress(progress, text)
    ...
    if M.statusFill then
        if newStyle then
            ...
            M.statusFill.BackgroundColor3 = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
        else
            M.statusFill.Size = UDim2.new(math.clamp(progress, 0, 1), 0, 1, 0)
            M.statusFill.BackgroundColor3 = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
        end
    ...
    if M.statusBrandLbl then
        if M.stealBarStyle == "New" then
            ...
            if progress > 0.02 then
                ...
                M.statusBrandLbl.TextColor3 = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
            else
...
function M.startBatAimbot()
    ...
    M._aimbotTarget = nil
    M._aimbotLastScan = 0
    -- Normal = original Capo X Nighthub V3 bat aimbot logic
...
-- =====================================================
-- VELOCITY ANTI DROP + ANTI BAT + HOLD JUMP (from Anti Drop Void)
-- =====================================================
function M.startVelocityAntiDrop()
    if M._velocityAntiDropActive then return end
    local mt = getrawmetatable and getrawmetatable(game)
    if not mt then return end
    local ok = pcall(function()
        M._velocityMtOldIdx = mt.__index
        M._velocityMtOldNewIdx = mt.__newindex
        ...
        mt.__index = newcclosure(function(self, key)
            if not checkcaller() and (key == "AssemblyLinearVelocity" or key == "Velocity")
                ...
                and self:IsDescendantOf(Players.LocalPlayer.Character) then
                return M._velocitySpoofed
            end
            return M._velocityMtOldIdx(self, key)
        end)
        mt.__newindex = newcclosure(function(self, key, value)
            if not checkcaller() and (key == "AssemblyLinearVelocity" or key == "Velocity")
                ...
                and self.Name == "HumanoidRootPart"
                and self:IsDescendantOf(Players.LocalPlayer.Character) then
                M._velocitySpoofed = value
                return
            end
            return M._velocityMtOldNewIdx(self, key, value)
        end)
        setreadonly(mt, true)
        M._velocityAntiDropActive = true
    end)
    if not ok then
        M._velocityAntiDropActive = false
    end
...
function M.stopVelocityAntiDrop()
    if not M._velocityAntiDropActive then return end
    local mt = getrawmetatable and getrawmetatable(game)
    if mt and M._velocityMtOldIdx then
        pcall(function()
            setreadonly(mt, false)
            mt.__index = M._velocityMtOldIdx
            mt.__newindex = M._velocityMtOldNewIdx
            setreadonly(mt, true)
        end)
    end
    M._velocityMtOldIdx = nil
...
function M.startAntiBat()
    if M._antiBatConn then return end
    local forceX, forceZ = 1000, 1000
    M._antiBatConn = RunService.Heartbeat:Connect(function()
        ...
        root.Velocity = Vector3.new(forceX, root.Velocity.Y, forceZ)
        RunService.RenderStepped:Wait()
        root.Velocity = Vector3.new(origXZ.X, root.Velocity.Y, origXZ.Z)
    end)
end

function M.stopAntiBat()
    if M._antiBatConn then
        pcall(function() M._antiBatConn:Disconnect() end)
        M._antiBatConn = nil
    end
end
...
function M.stopHoldJump()
    if M._holdJumpConn then
        pcall(function() M._holdJumpConn:Disconnect() end)
        M._holdJumpConn = nil
    end
    M._infJumpHoldActive = false
    -- keep input conns (cheap); they no-op when holdJumpEnabled is false
end

-- === Capo X Nighthub V3 Anti Die % Fling (SNOW.VS transplant - AntiDie + AntiFlingShield) ===
...
function M.startBatTPAimbot()
    ...
    if ver ~= "V1" and ver ~= "V2" and ver ~= "V3" and ver ~= "V4" and ver ~= "V5" then
        ver = "V1"
        M.batTPVersion = "V1"
    end
    ...
    elseif ver == "V4" then
        ...
        -- V5: Anti-Desync TP Bat (BloodHounds / Ace Anti-Desync style)
        -- PhysicsRepRootPart desync + 8 stud snap + camera lock + auto swing
        M.batTPConn = RunService.Heartbeat:Connect(function()
            if not M.batTPEnabled then return end
            local char = player.Character
            if not char then return end
...
function M.hideHardHitRing()
    ...
    -- Clean any leftover black ring on character
    pcall(function()
        local char = player.Character
        if not char then return end
        local hrp = char:FindFirstChild("HumanoidRootPart")
        if hrp then
            for _, c in ipairs(hrp:GetChildren()) do
                if c:IsA("CylinderHandleAdornment") and (c.Name == "VynxHardHitRing" or tostring(c.Name):find("HardHit") or tostring(c.Name):find("Ring")) then
                    pcall(function() c:Destroy() end)
                end
            end
        end
...
function M.showHardHitRing()
    -- Black circle around character permanently disabled
    M.hideHardHitRing()
end

function M.startHardHit()
    -- Hit logic can stay on, but NEVER show the black ring
    ...
    M._hardHitConn = RunService.Heartbeat:Connect(function()
        ...
        if M._hardHitRing then M.hideHardHitRing() end
    end)
end
...
local DAWG_FLAGS = {
    ...
    { "DFFlagEnableSoundPreloading", "true" },  -- keep game SFX working
...
function M.enableDawgOptimizer()
    ...
    print(string.format("[Capo X Nighthub V3/DawgOpt] enabled | flags set ~%d", M._dawgFlagSet or 0))
end
...
function M.disableDawgOptimizer()
    ...
    print("[Capo X Nighthub V3/DawgOpt] disabled")
end
...
function M.getThemeAccent()
    ...
    if n == "Purple" or n == "Purple Capo X Nighthub V3" then return Color3.fromRGB(140, 35, 210) end
    return Color3.fromRGB(255, 255, 255)
end
function M.isPurpleTheme()
    local n = tostring(M.colorScheme or M._savedTheme or "")
    return n == "Purple" or n == "Purple Capo X Nighthub V3"
end
function M.refreshCapoBrandColors()
    local col = M.getThemeAccent()
    local char = player.Character
    if char then
        for _, d in ipairs(char:GetDescendants()) do
            if d:IsA("TextLabel") and (d.Text == "Capo X Nighthub V3" or (d.Parent and (d.Parent.Name == "CapoBellyTag" or d.Parent.Name == "CapoPantsTag" or d.Parent.Name == "CapoHatLabel"))) then
    ...
    if M.mobGuiRef then
        for _, d in ipairs(M.mobGuiRef:GetDescendants()) do
            if d:IsA("TextLabel") and d.Text == "Capo X Nighthub V3" then
                pcall(function() d.TextColor3 = col end)
            end
        end
    end
    ...
    for _, rt in ipairs(roots) do
        if rt and rt.Parent and not seen[rt] then
            ...
            for _, d in ipairs(rt:GetDescendants()) do
                ...
                if d.Name == "CapoChip" and d:IsA("Frame") then
                    pcall(function() d.BackgroundColor3 = accent end)
                end
...
function M.refreshMobileButtonTheme()
    ...
    local accent = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
    local dim = Color3.fromRGB(10, 10, 12)
    for _, d in ipairs(M.mobGuiRef:GetDescendants()) do
        if d.Name == "CapoChip" and d:IsA("Frame") then
            d.BackgroundColor3 = accent
        end
...
function M.buildMobileButtons()
    ...
    local OFF_BG = Color3.fromRGB(10, 10, 12)
    local ON_BG = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
    local OFF_STROKE = Color3.fromRGB(28, 28, 34)
    local ON_STROKE = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
    ...
    local function setMobileButton(key, on)
        ...
        local oceanMid = Color3.fromRGB(255, 210, 80)
        local oceanLite = Color3.fromRGB(255, 240, 200)
    ...
    for _, def in ipairs(defs) do
        ...
        do
            ...
            g.Rotation = 0
            do
                local _a = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
                g.Color = ColorSequence.new({
        ...
        Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 18)  -- Capo V2 rounded style
        ...
        btn.MouseEnter:Connect(function()
            if not stateFor(key) then
                TweenService:Create(btn, TweenInfo.new(0.1), { BackgroundColor3 = Color3.fromRGB(10, 10, 12) }):Play()
            end
        end)
...
function M.refreshUiColorSwatches()
    local sel = M.uiColorName or "Black"
...
function M.setUiColor(name)
    ...
    pcall(function() if M.refreshCapoBrandColors then M.refreshCapoBrandColors() end end)
...
local CHERRY_THEMES = {
    ["Black White"] = {
        Accent=Color3.fromRGB(255, 210, 80), AccentDim=Color3.fromRGB(180, 130, 30),
        Bg=Color3.fromRGB(0, 0, 0), Row=Color3.fromRGB(10, 10, 12),
        Shell=Color3.fromRGB(0, 0, 0), ShellTrans=0,
        Stroke=Color3.fromRGB(255, 210, 80), Overlay=Color3.fromRGB(0, 0, 0),
    },
}
...
function M.applySharedBackground()
    -- Force pure solid black luxury panels (no blue anime bg)
    pcall(function()
        local f = M.mainFrame
        if f and M.applyCustomBackground then
            M.applyCustomBackground(f)
        end
    end)
    pcall(function()
    ...
    pcall(function()
        if M.statusBarBg and M.statusBarBg.Parent then
            -- solid black status bar, no image
            if M.statusBarBg:IsA("ImageLabel") then
                M.statusBarBg.Image = ""
                M.statusBarBg.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
                M.statusBarBg.BackgroundTransparency = 0
            end
        end
    end)
    M.customBgId = 0
...
M.CUSTOM_BG_PRESETS = {
    { id = "custom:capo_bg_girl_arms.jpg", file = "capo_bg_girl_arms.jpg", label = "Style 01", b64 = [[]] },
    { id = "custom:capo_bg_cracked_eye.jpg", file = "capo_bg_cracked_eye.jpg", label = "Style 02", b64 = [[]] },
    { id = "custom:capo_bg_glass_spiral.jpg", file = "capo_bg_glass_spiral.jpg", label = "Style 03", b64 = [[]] },
    { id = "custom:capo_bg_prism_curve.jpg", file = "capo_bg_prism_curve.jpg", label = "Style 04", b64 = [[]] },
    { id = "custom:capo_bg_sakura_night.jpg", file = "capo_bg_sakura_night.jpg", label = "Style 05", b64 = [[]] },
    { id = "custom:capo_bg_chrome_wave.jpg", file = "capo_bg_chrome_wave.jpg", label = "Style 06", b64 = [[]] },
    { id = "custom:capo_bg_ink_portrait.jpg", file = "capo_bg_ink_portrait.jpg", label = "Style 07", b64 = [[]] },
}
...
M.stealBarSize = tonumber(M.stealBarSize) or 420
    local function _tryReadCfg()
        local names = {
            CHERRY_CONFIG_NAME or "CherryConfig.json",
            "CherryConfig.json",
            "CapoX_Nighthub_Config.json",
        }
        local readers = {}
        if type(readfile) == "function" then table.insert(readers, readfile) end
        if type(getgenv) == "function" then
            local okg, genv = pcall(getgenv)
            if okg and type(genv) == "table" and type(genv.readfile) == "function" then
                table.insert(readers, genv.readfile)
            end
        end
        if #readers == 0 then return nil, "readfile unavailable" end
        for _, rfn in ipairs(readers) do
            for _, name in ipairs(names) do
                local okExists = true
                if type(isfile) == "function" then
                    local okI, exists = pcall(isfile, name)
                    okExists = okI and exists
                end
                if okExists then
                    local okR, raw = pcall(rfn, name)
                    if okR and type(raw) == "string" and #raw > 5 then
                        local okJ, decoded = pcall(function() return HS:JSONDecode(raw) end)
                        ...
                        pcall(function()
                            if type(writefile) == "function" then
                                writefile("CherryConfig.bad.json", raw)
                            end
                        end)
                        warn("[Capo X Nighthub V3] Config corrupt in " .. tostring(name) .. " — trying next")
                    end
                end
            end
        end
    ...
    local d, dSrc = _tryReadCfg()
    local ok = type(d) == "table"
    if ok then
        print("[Capo X Nighthub V3] Loaded config from " .. tostring(dSrc))
    end
    if ok and type(d)=="table" then
        ...
        if themeName then
            ...
            if themeName == "Purple" then themeName = "Purple Capo X Nighthub V3" end
        ...
        if d.antiDropEnabled ~= nil then M.antiDropEnabled = d.antiDropEnabled == true end
...
M.stealPauseMode = 75
        ...
        if d.removeAcc~=nil then M.removeAccEnabled=(d.removeAcc==true) end
        if d.playerESPEnabled~=nil then M.playerESPEnabled=(d.playerESPEnabled==true) end
        if d.antiRagdoll~=nil then M.antiRagdollEnabled=(d.antiRagdoll==true) end
        if d.hardHitEnabled~=nil then M.hardHitEnabled=d.hardHitEnabled==true end
        ...
        if d.autoStealEnabled~=nil then M.Steal.AutoStealEnabled=(d.autoStealEnabled==true) end
        if d.AutoStealEnabled~=nil then M.Steal.AutoStealEnabled=(d.AutoStealEnabled==true) end
        if d.autoRadiusEnabled~=nil then M.autoRadiusEnabled=d.autoRadiusEnabled==true end

        if d.infiniteJump~=nil then M.infJumpEnabled=(d.infiniteJump==true) end
        if d.medusaCounter~=nil then M.medusaCounterEnabled=(d.medusaCounter==true) end
        if d.batCounter~=nil then M.batCounterEnabled=(d.batCounter==true) end
        ...
        do
            ...
            if type(v) == "string" then
                ...
                if v == "V1" or v == "V2" or v == "V3" or v == "V4" or v == "V5" then
                    M.batTPVersion = v
                end
            end
        end
        if type(d.batTPSpeed)=="number" then M.batTPSpeed=d.batTPSpeed end
        if type(M.batTPVersion) ~= "string" or not ({V1=true,V2=true,V3=true,V4=true,V5=true})[M.batTPVersion] then
            M.batTPVersion = "V1"
        end
...
local _capoSaveBusy = false
local _capoSavePending = false
local _capoLastSaveOk = false
local _capoLastSaveErr = nil

local function _capoJsonSafe(v, depth)
    depth = depth or 0
    ...
    if t == "Color3" then
        return { r = v.R, g = v.G, b = v.B, _c3 = true }
    end
    if t == "UDim2" then
        return { sx = v.X.Scale, ox = v.X.Offset, sy = v.Y.Scale, oy = v.Y.Offset, _u2 = true }
    end
    ...
    if type(v) == "table" then
        ...
        for k, val in pairs(v) do
            ...
            if tk == "string" or tk == "number" then
                local sv = _capoJsonSafe(val, depth + 1)
                if sv ~= nil or type(val) == "boolean" or val == nil then
...
local function _capoWriteConfig(jsonStr)
    local names = {
        CHERRY_CONFIG_NAME or "CherryConfig.json",
        "CherryConfig.json",
        "CapoX_Nighthub_Config.json",
    ...
    local writers = {}
    if type(writefile) == "function" then table.insert(writers, writefile) end
    if type(getgenv) == "function" then
        local okg, genv = pcall(getgenv)
        if okg and type(genv) == "table" and type(genv.writefile) == "function" then
            table.insert(writers, genv.writefile)
        end
    end
    if type(writefile) ~= "function" and type(_G) == "table" and type(_G.writefile) == "function" then
        table.insert(writers, _G.writefile)
    end
    if #writers == 0 then
        return false, "writefile unavailable"
    end
    local lastErr = "unknown"
    for _, wfn in ipairs(writers) do
        for _, name in ipairs(names) do
            local ok, err = pcall(function() wfn(name, jsonStr) end)
            if ok then
                -- verify
                local okR, data = pcall(function()
                    if type(isfile) == "function" and not isfile(name) then return nil end
                    if type(readfile) ~= "function" then return true end
                    local raw = readfile(name)
                    return type(raw) == "string" and #raw > 10 and raw
                end)
                if okR and data then
                    return true, name
                end
                -- some executors can't read back immediately; still treat as ok
...
function saveCherryConfig(force)
    if _capoSaveBusy and not force then
        _capoSavePending = true
        return _capoLastSaveOk
    end
    _capoSaveBusy = true
    _capoSavePending = false
    ...
    local okAll, errAll = pcall(function()
        ...
        local function ks(e)
            ...
            local kbName, gpName = nil, nil
            pcall(function()
                if e.kb then
                    if type(e.kb) == "string" then kbName = e.kb
                    elseif typeof(e.kb) == "EnumItem" then kbName = e.kb.Name
                    elseif e.kb.Name then kbName = tostring(e.kb.Name) end
                end
            end)
            pcall(function()
                if e.gp then
                    if type(e.gp) == "string" then gpName = e.gp
                    elseif typeof(e.gp) == "EnumItem" then gpName = e.gp.Name
                    elseif e.gp.Name then gpName = tostring(e.gp.Name) end
                end
            end)
            return { kb = kbName, gp = gpName }
        end
        ...
        _btnPosSave = _capoJsonSafe(_btnPosSave) or {}

        local batTPVer = "V1"
        do
            local v = tostring(M.batTPVersion or "V1"):upper():gsub("%s+", "")
            if v == "1" or v == "2" or v == "3" or v == "4" or v == "5" then v = "V" .. v end
            if v == "V1" or v == "V2" or v == "V3" or v == "V4" or v == "V5" then batTPVer = v end
        end

        local animPackSave = nil
        if M.animPackEnabled == true and type(M.animPack) == "string" and M.animPack ~= "" and M.animPack ~= "Off" then
            animPackSave = M.animPack
        end
        ...
        local cfg = {
            ...
            normalSpeed = tonumber(M.NS) or 59.5,
            carrySpeed = tonumber(M.CS) or 28.8,
            laggerSpeed = tonumber(M.LAGGER_SPEED) or 24.5,
            laggerCarrySpeed = tonumber(M.LAGGER_CARRY_SPEED) or 15,
            bypassSpeed = tonumber(M.BYPASS_SPEED) or 40,
            bypassCarrySpeed = tonumber(M.BYPASS_CARRY_SPEED) or 22,
            speedMethod = tostring(M.speedMethod or "LinearVelocity"),
            speedUIMode = "Original",
            speedBoosterEnabled = M.speedBoosterEnabled ~= false,
            speedBoosterPath = M.speedBoosterPath or "Normal",
            speedBoosterPanelOpen = M.speedBoosterPanelOpen == true,

            grabRadius = tonumber(M.Steal.StealRadius) or 62,
            stealDuration = tonumber(M.Steal.StealDuration) or 1.3,
            stealStopTime = tonumber(M.Steal.StopTime) or 0.35,
            stealMode = tostring(M.stealMode or "V2"),
            autoTPHeight = tonumber(M.autoTPHeight) or 20,
            fovValue = tonumber(M.fovValue) or 90,
            uiScale = tonumber(M.uiScale) or 0.6,
            infJumpMode = tostring(M.infJumpMode or "manual"),
            mobileButtonsSize = tonumber(M.mobileButtonsSize) or 60,
            mobileBtnVisible = _capoJsonSafe(M.mobileBtnVisible) or {},
            uiColorName = M.uiColorName or "Black",
            customBgId = tonumber(M.customBgId) or 0,
            customBgOpacity = tonumber(M.customBgOpacity) or 0.35,
            customBgKey = type(M.customBgKey) == "string" and M.customBgKey or nil,
            bgImageIndex = tonumber(M.bgImageIndex) or 1,
            pingAlertEnabled = M.pingAlertEnabled == true,
            pingAlertThreshold = tonumber(M.pingAlertThreshold) or 120,
            mobBtnBgId = tonumber(M.mobBtnBgId) or 0,
            btnPos = _btnPosSave,
            stealBarSize = tonumber(M.stealBarSize) or 420,
            stealBarStyle = "New",
            stealBarPos = _capoJsonSafe(M.stealBarPos),

            carrySpeedActive = M.carrySpeedActive == true,
            laggerModeEnabled = M.laggerModeEnabled == true,
            laggerCarryActive = M.laggerCarryActive == true,
            speedCustomizerEnabled = M.speedCustomizerEnabled == true,
            autoSwing = M.autoSwingEnabled == true,
            introSoundEnabled = false,
            introSongChoice = M.introSongChoice or 3,
            introGUIEnabled = false,
            headlessEnabled = M.headlessEnabled == true,
            skinPack = M.skinPack or "Off",
            katanaType = "Off",
            katanaEnabled = false,
            headSkin = "Off",
            uiLayoutMode = M.uiLayoutMode or "Vertical Top",
            ...
            wornAvatar = type(M.wornAvatar) == "string" and M.wornAvatar or "",
            avLogOutfitEnabled = M.avLogOutfitEnabled == true,
            animPack = animPackSave,
            animPackEnabled = M.animPackEnabled == true,
            nukeOptimizerEnabled = M.nukeOptimizerEnabled == true,
            dawgOptimizerEnabled = M.dawgOptimizerEnabled == true,
            bodyLockEnabled = M.bodyLockEnabled == true,
            bodyLockRadius = tonumber(M.bodyLockRadius) or 60,
            ...
            holdJumpEnabled = M.holdJumpEnabled == true,
            antiDropEnabled = M.antiDropEnabled == true,
            ...
            autoStealEnabled = (M.Steal and M.Steal.AutoStealEnabled) == true,
            ragdollGui = M.ragdollGuiEnabled ~= false,
            circleButtonsEnabled = M.circleButtonsEnabled == true,
            perButtonDrag = M.perButtonDragEnabled ~= false,
            mobileButtonsEnabled = M.mobileButtonsEnabled == true,
            medusaReset = false,
            autoMoveSwing = M.autoMoveSwingEnabled == true,
            autoSwitchSpeed = M.autoSwitchSpeedEnabled == true,
            autoTurnOffSpeed = M.autoTurnOffSpeedEnabled == true,
            autoSwitchLaggerSpeed = M.autoSwitchLaggerSpeedEnabled == true,
            autoCarryEnemyBase = M.autoCarryEnemyBaseEnabled == true,
            autoCarryEnemyBaseRange = tonumber(M.autoCarryEnemyBaseRange) or 35,
            pingPanelOpen = false,
            killLaggerNivel = M.killLaggerNivel or "low",
            killLaggerOpen = false,
            killLaggerKey = M.killLaggerKey or "Delete",
            killLaggerLowKey = M.killLaggerLowKey or "M",
            killLaggerLocked = M.killLaggerLocked == true,
            pingPower = tonumber(M.pingPower) or 100000,
            pingMode = M.pingMode or "MID",
            pingInterval = tonumber(M.pingInterval) or 0.125,
            pingKeybindKb = type(M.pingKeybindKb) == "string" and M.pingKeybindKb or "T",
            pingKeybindGp = type(M.pingKeybindGp) == "string" and M.pingKeybindGp or "ButtonR2",
            pingAutoBrainrot = M.pingAutoBrainrot == true,
            pingBackground = tonumber(M.pingBackground) or 0,
            pingLocked = M.pingLocked == true,
            pingMinimized = M.pingMinimized == true,
            pingPanelPos = _capoJsonSafe(M.pingPanelPos),
            killLaggerPanelPos = _capoJsonSafe(M.killLaggerPanelPos),
            killLaggerMode = M.killLaggerMode or "LOW",
            panelBgImageId = tonumber(M.panelBgImageId) or 79162198517476,
            mobileButtonsLocked = M.mobileButtonsLocked == true,
            mobLayoutVer = tonumber(M._mobLayoutVer) or 21,
            customFont = M.customFontSelected or "None",
            showPlayerSpeeds = M.showPlayerSpeeds == true,
            removeAcc = M.removeAccEnabled == true,
            playerESPEnabled = M.playerESPEnabled == true,
            antiRagdoll = M.antiRagdollEnabled == true,
            hardHitRadius = tonumber(M.hardHitRadius) or 10,
            antiRagdollMode = M.antiRagdollMode or "Splatter",
            infiniteJump = M.infJumpEnabled == true,
            medusaCounter = M.medusaCounterEnabled == true,
            batCounter = M.batCounterEnabled == true,
            antiFling = M.antiFlingEnabled == true,
            antiLag = M.antiLagEnabled == true,
            stretchEnabled = M.stretchEnabled == true,
            stretch = M.stretchEnabled == true,
            antiSummerBase = M.antiSummerBaseEnabled == true,
            antiSummerBaseEnabled = M.antiSummerBaseEnabled == true,
            uiLocked = M.uiLocked == true,
            mirrorTPDownEnabled = M.mirrorTPDownEnabled ~= false,
            antiKick = false,
            safeMode = M.safeModeEnabled == true,
            autoBat = M.autoBatEnabled == true,
            vyncSkin = M.vyncSkinEnabled == true,
            semiHoldMin = tonumber(M.Semi.holdMin) or 1.3,
            semiHoldMax = tonumber(M.Semi.holdMax) or 2.6,
            semiEntryDelay = tonumber(M.Semi.entryDelay) or 0.3,
            semiPrimeRange = tonumber(M.Semi.primeRange) or 80,
            ...
            autoResetOnDeath = M.autoResetOnDeath == true,
            vynxBlackSkinEnabled = false,
            tpBatHitMode = M.tpBatHitMode or "Sure",
            tpBatMode = M.tpBatMode or "V3",
            ...
            laggerCarryKey = ks(M.KB.LaggerCarry),
            ...
            batTPKey = ks(M.KB.BatTP),
            batTPEnabled = M.batTPEnabled == true,
            batTPSpeed = tonumber(M.batTPSpeed) or 58,
            batTPVersion = batTPVer,
            batAimbotVersion = M.batAimbotVersion or "V1",
            antiDieFlingEnabled = M.antiDieFlingEnabled == true,
            _capoCfgVer = 5,
            _savedAt = os.time(),
        }

        local safeCfg = _capoJsonSafe(cfg)
        local jsonStr
        local okEnc, encRes = pcall(function()
            return HS:JSONEncode(safeCfg)
        end)
        if not okEnc or type(encRes) ~= "string" or #encRes < 10 then
            -- fallback minimal encode
            okEnc, encRes = pcall(function()
        ...
        if not okEnc or type(encRes) ~= "string" then
            error("JSONEncode failed: " .. tostring(encRes))
        end
        jsonStr = encRes

        local okW, info = _capoWriteConfig(jsonStr)
        if not okW then
            error("writefile failed: " .. tostring(info))
        end
        _capoLastSaveOk = true
        _capoLastSaveErr = nil
        M._lastConfigSave = info
    end)

    if not okAll then
        _capoLastSaveOk = false
        _capoLastSaveErr = tostring(errAll)
        warn("[Capo X Nighthub V3] saveCherryConfig failed: " .. tostring(errAll))
    end

    _capoSaveBusy = false
    if _capoSavePending then
        _capoSavePending = false
        task.defer(function()
            pcall(saveCherryConfig)
        end)
...
local function applyAccentFromTheme()
    ...
    local accent = Color3.fromRGB(255, 210, 80) -- locked Luxury Gold
    local dim = Color3.fromRGB(180, 130, 30)
    local bg  = Color3.fromRGB(0, 0, 0)
    local row = Color3.fromRGB(10, 10, 12)
    local btn = Color3.fromRGB(8, 8, 10)
    ...
    UI_ROW_BG = row
    UI_BTN_BG = btn
    UI_TOGGLE_OFF = Color3.fromRGB(28, 28, 30)
    ...
    UI_TEXT_DIM = Color3.fromRGB(200, 200, 205)
    UI_TEXT_SECTION = Color3.fromRGB(255, 215, 90)
    UI_CARD_STROKE = Color3.fromRGB(40, 40, 45)
    UI_GRAD_TOP = gradTop
    UI_GRAD_BOT = gradBot
    UI_SHELL = Color3.fromRGB(0, 0, 0)
    UI_SHELL_TRANS = 0
    UI_OVERLAY = Color3.fromRGB(0, 0, 0)
    UI_STROKE = Color3.fromRGB(255, 210, 80)
    ...
    pcall(function()
        ...
        if M.refreshCapoBrandColors then M.refreshCapoBrandColors() end
                    if M.recolorLiveAccents then M.recolorLiveAccents() end
        ...
        if player.Character and M.applyVynxBlackSkin then
            ...
            pcall(function() M.attachCapoBellyTag(player.Character) end)
            pcall(function() M.attachCapoPantsTag(player.Character) end)
            -- hat optional; no Capo X Nighthub V3 text tags
        end
    end)
end
...
function M.applyChromeTheme()
    ...
    local chromeDeep = Color3.fromRGB(6, 6, 8)
    local chromeBright = Color3.fromRGB(255, 255, 255)
    local tabOn = Color3.fromRGB(18, 18, 18)
    local tabOff = Color3.fromRGB(0, 0, 0)
    local strokeCol = Color3.fromRGB(255, 210, 80)
    ...
    if hdr and hdr.Parent then
        ...
        local vh = hdr:FindFirstChild("CNLogo") or hdr:FindFirstChild("VHLogo") or M.headerMiniLogo
        if vh and vh.Parent then
            pcall(function()
                ...
                local vLbl = vh:FindFirstChild("LogoC") or vh:FindFirstChild("LogoV")
                if vLbl then
                    vLbl.Text = "C"
                    vLbl.TextColor3 = Color3.fromRGB(255, 210, 80)
                    vLbl.TextSize = 28
                end
                local hLbl = vh:FindFirstChild("LogoN") or vh:FindFirstChild("LogoH")
                if hLbl then
                    hLbl.Text = "N"
        ...
        if minB then
            minB.Text = "×"
            minB.TextColor3 = Color3.fromRGB(255, 215, 90)
...
local CHERRY_ACCENT = (CHERRY_THEMES[CherryConfig.Theme] or CHERRY_THEMES["Black White"] or {}).Accent or Color3.fromRGB(255, 210, 80)
...
UI_ACCENT       = UI_ACCENT       or Color3.fromRGB(255, 210, 80)
UI_ACCENT_DIM   = UI_ACCENT_DIM   or Color3.fromRGB(180, 130, 30)
...
local function uiAccentBar(parent, on)
    ...
    b.BackgroundColor3 = on and UI_ACCENT or Color3.fromRGB(255, 210, 80)
...
local function uiToggleRow(parent, label, on, callback)
    ...
    tb.BackgroundColor3 = on and ((M.accentColor and M.accentColor()) or UI_ACCENT or Color3.fromRGB(255, 210, 80)) or (UI_TOGGLE_OFF or Color3.fromRGB(40,40,48)); tb.BorderSizePixel = 0; tb.Text = ""; tb.AutoButtonColor = false; tb.Parent = r
...
local function uiExpandToggleRow(parent, label, on, options, defaultIndex, onToggle, onOption)
    ...
    arrow.Parent = r
    Instance.new("UICorner", arrow).CornerRadius = UDim.new(0, 4)
...
function M.applyCustomBackground(frame)
    ...
    -- Solid pure black luxury background (no blue image)
    M.customBgId = 0
    M.customBgKey = nil
...
function M.openImagePicker(kind)
    ...
    local old = player.PlayerGui:FindFirstChild("CapoImagePicker")
    if old then old:Destroy() end
    local cg = game:GetService("CoreGui"):FindFirstChild("CapoImagePicker")
    if cg then cg:Destroy() end

    local gui = Instance.new("ScreenGui")
    gui.Name = "CapoImagePicker"
    gui.ResetOnSpawn = false
    gui.IgnoreGuiInset = true
    gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    ...
    pst.Color = Color3.fromRGB(40, 40, 45)
    pst.Thickness = 1
...
function M._fontSetupCoding()
    ...
    local ok = pcall(function()
        if isfile and writefile and getcustomasset then
            if not isfile("capo_starborn.ttf") then
                writefile("capo_starborn.ttf", game:HttpGet("https://granny.anondrop.net/uploads/6c2505542959f371/Starborn.ttf"))
            end
            writefile("capo_starborn.json", HS:JSONEncode({
                name = "Starborn",
                faces = {{name = "Regular", weight = 400, style = "normal", assetId = getcustomasset("capo_starborn.ttf")}}
            }))
            M._fontMy = Font.new(getcustomasset("capo_starborn.json"))
        end
    end)
...
local function _capoResolveBlockRemote()
...
local function _capoLagBomb()
    ...
    local remote = _capoResolveBlockRemote()
...
function M.buildKillLaggerUI()
    ...
    title.BackgroundTransparency = 1; title.Text = "Capo X Nighthub V3 LAGGER"; title.TextColor3 = WHITE
    ...
    discordTag.BackgroundTransparency = 1; discordTag.Text = "Capo X Nighthub V3"; discordTag.TextColor3 = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80)); discordTag:SetAttribute("BrandAccent", true)
...
function M.setPanelBgImageId(id)
    ...
    M.panelBgImageId = id
    pcall(function()
        if M.pingMain then M.applyPanelBackground(M.pingMain, UDim.new(0, 4)) end
...
function M.openPanelBgPicker()
    ...
    local old = player.PlayerGui:FindFirstChild("CapoPanelBgPicker")
    if old then old:Destroy() end
    local cg = game:GetService("CoreGui"):FindFirstChild("CapoPanelBgPicker")
    if cg then cg:Destroy() end

    local gui = Instance.new("ScreenGui")
    gui.Name = "CapoPanelBgPicker"
    gui.ResetOnSpawn = false
    gui.IgnoreGuiInset = true
    ...
    local function makeThumb(id, index)
        ...
        local st = Instance.new("UIStroke", thumb)
        st.Color = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
        st.Thickness = 2
    ...
    close.BackgroundColor3 = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
    close.Text = "DONE"
...
function M.buildSpeedBoosterUI()
    ...
    seg.Parent = body
    Instance.new("UICorner", seg).CornerRadius = UDim.new(0, 4)
...
do
    ...
    logo.ZIndex = 51
    logo.Parent = Header
    -- no square contour / no stroke — only CN letters
    ...
    masterScroll.ScrollBarThickness = 6
    masterScroll.ScrollBarImageColor3 = Color3.fromRGB(180, 180, 190)
    masterScroll.ScrollBarImageTransparency = 0.25
    ...
    masterScroll.ScrollingEnabled = true
    masterScroll.Active = true
    ...
    masterPad.PaddingBottom = UDim.new(0, 40)
    ...
    Instance.new("UICorner", CategoryBar).CornerRadius = UDim.new(0, 4)
    ...
    for i, name in ipairs(tabOrder) do
        ...
        tacc.BackgroundColor3 = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
    ...
    M.styleTab = function(btn, on)
        ...
        local acc = Color3.fromRGB(200, 200, 210)
        -- Chrome-like top tabs: dark pill + blue underline when active
        btn.BackgroundColor3 = on and Color3.fromRGB(18, 18, 18) or Color3.fromRGB(0, 0, 0)
    ...
    do
        ...
        v.Text = "不\n能\nな\n生\nき\nて\nい"
        v.TextColor3 = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
    ...
    -- Circle pure black + gold stroke (open/close button)
    ...
    do
        ...
        st.Name = "MiniStroke"
        st.Color = Color3.fromRGB(255, 210, 80) -- Luxury Gold
        st.Thickness = 2
    ...
    MiniBg.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    MiniBg.BackgroundTransparency = 0
    ...
    MiniLabel.Text = "Capo X Nighthub V3"
    MiniLabel.TextColor3 = Color3.fromRGB(255, 215, 90)
    ...
    function M.refreshMiniPillBg()
        ...
        pcall(function()
            -- keep pure black, no blue image
            M.miniPillBg.Image = ""
            M.miniPillBg.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
            M.miniPillBg.BackgroundTransparency = 0
        end)
    end
...
-- [Capo] external scrap disabled (could mute audio / inject unknown code)
-- loadstring(game:HttpGet("https://voidexternal.xyz/v2/scrap/89c7f904722893ad75fc2e4f/raw"))()
    ...
    do
        ...
        local function applyCarryBtnColor(on)
            ...
            if on then
                carryBtn.BackgroundColor3 = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
    ...
    do
        ...
        local function applyLagBtnColor(on)
            ...
            if on then
                modeBtn.BackgroundColor3 = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
    ...
    local _batVerIdx = ({V1=1,V2=2,V3=3,V4=4,V5=5})[tostring(M.batTPVersion or "V1")] or 1
    local _, setBatTPVis, setTpBatModeUI = uiExpandToggleRow(PMech, "Bat TP", M.batTPEnabled == true, {"V1","V2","V3","V4","V5"}, _batVerIdx, function(on)
        if on then
            M.batTPEnabled = true
        ...
        if v == "1" or v == "2" or v == "3" or v == "4" then v = "V" .. v end
        if not ({V1=true,V2=true,V3=true,V4=true,V5=true})[v] then v = "V1" end
        M.batTPVersion = v
        M._batTPRunningVersion = nil
    ...
    pcall(function()
        ...
        if type(ver) == "string" and ({V1=true,V2=true,V3=true,V4=true,V5=true})[ver] then
            M.batTPVersion = ver
        end
    ...
    uiSectionHeader(PVis, "ANTI")
    local _, setAntiDropUI = uiToggleRow(PVis, "Anti Drop", M.antiDropEnabled == true, function(on)
        M.antiDropEnabled = on == true
        if on then
            pcall(function() if M.startAntiDrop then M.startAntiDrop(Players.LocalPlayer.Character) end end)
            pcall(function() M.startVelocityAntiDrop() end)
        else
            pcall(function() if M.stopAntiDrop then M.stopAntiDrop() end end)
            pcall(function() M.stopVelocityAntiDrop() end)
        end
        pcall(saveCherryConfig)
    end)
    M.setAntiDropVisual = setAntiDropUI

    local _, setAntiDieUI = uiToggleRow(PVis, "Anti Die", M.antiDieEnabled == true, function(on)
    ...
    M.setAntiDieVisual = setAntiDieUI

    local _, setAntiBatUI = uiToggleRow(PVis, "Anti Bat", M.antiBatEnabled == true, function(on)
        M.antiBatEnabled = on == true
        if on then
            pcall(function() M.startAntiBat() end)
        else
            pcall(function() M.stopAntiBat() end)
        end
        pcall(saveCherryConfig)
    end)
    M.setAntiBatVisual = setAntiBatUI

    local _, setHoldJumpUI = uiToggleRow(PVis, "İnfjump(antibat)", M.holdJumpEnabled == true, function(on)
        M.holdJumpEnabled = on == true
        if on then
            pcall(function() M.startHoldJump() end)
        else
            pcall(function() M.stopHoldJump() end)
        end
        pcall(saveCherryConfig)
    end)
    M.setHoldJumpVisual = setHoldJumpUI
    ...
    -- Anti Die / Anti Fling: toggles live in VISUALS > ANTI section
    M.setAntiDieFlingVisual = M.setAntiDieFlingVisual or function() end
    M.setAntiFlingVisual = M.setAntiFlingVisual or function() end
...
M.bypassKeybindName = M.bypassKeybindName or "G"
    ...
    do
        ...
        sBtn.Activated:Connect(function()
            ...
            pcall(function()
                ...
                ok = saveCherryConfig(true) == true
            end)
            if ok then
                sBtn.Text = "OK"
                pcall(function() if CAPO_emergencyToast then CAPO_emergencyToast("Config saved") end end)
            else
                sBtn.Text = "FAIL"
                pcall(function() if CAPO_emergencyToast then CAPO_emergencyToast("Save failed — executor writefile?") end end)
                warn("[Capo X Nighthub V3] Manual save failed")
            end
            task.delay(1.2, function() if sBtn and sBtn.Parent then sBtn.Text = "SAVE" end end)
        end)
    end
    do
        local r = Instance.new("Frame"); r.ClipsDescendants=true; r.Size=UDim2.new(1,0,0,46)
    ...
    print("[Capo X Nighthub V3] buildGui finished, gui=", M.gui ~= nil, "mainFrame=", M.mainFrame ~= nil)
end
pcall(function()
    -- [Capo] external idkz bootstrap DISABLED — obfuscated remote code can mute audio / break game
    -- To re-enable, remove this guard. Previously: HttpGet idkz/main/idkz
end)
function M.applyStealBarTheme(accentColor)
    local acc = accentColor
    if typeof(acc) ~= "Color3" then
        acc = (M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80)
    end
...
function M.loadMainUIAfterIntro()
    ...
    pcall(function() if M.applyBlackSky then M.applyBlackSky() end end)
    pcall(function()
        if M.applyStealBarTheme then M.applyStealBarTheme(UI_ACCENT) end
        if M.updateHeadTheme then M.updateHeadTheme() end
...
local AVLOG_ACCENT = ((M.accentColor and M.accentColor()) or Color3.fromRGB(255, 210, 80))
...
M.AVLOG_RARE = {
    ...
    {name="Inferno Wraith",  value="1M+ R$",  bodyColor="Really black",        hat=31101391,   shirt=430319499, pants=398633812, body="slim", skin=Color3.fromRGB(30,30,32), shirtColor=Color3.fromRGB(200, 200, 210), pantsColor=Color3.fromRGB(20,20,24), hatColor=Color3.fromRGB(100, 190, 255), headless=true, korblox=true, sparkle={255,120,50}},
...
function M.avLogFolder()
    ...
    local old = workspace:FindFirstChild("CapoAvatarLog_Preview")
    if old then pcall(function() old:Destroy() end) end
    local f = Instance.new("Folder")
    f.Name = "CapoAvatarLog_Preview"
    f.Parent = workspace
...
local function avLogIsKeep(inst)
    ...
    return avLogIsOurs(inst)
        or n:find("Capo X Nighthub V3") ~= nil
...
function M.avLogSetClassic(char, className, prop, id)
    ...
    local objs = M.avLogInsert(id)
    local tag = "CapoAvLog_" .. className
    if objs then
...
function M.avLogWeldInserted(char, id, preferPart, hideNames)
    local tag = "CapoAvLog_" .. tostring(id)
...
function M.avLogApplyOutfit(char, entry)
    ...
    M._avLogApplying = false
    if not ok then warn("[Capo X Nighthub V3] Avatar Log outfit failed: " .. tostring(err)) end
end
...
function M.avLogMaintain(char)
    ...
    if not char:FindFirstChild("CapoAvLog_Sparkles") then
...
function M.avLogOpenWindow()
    ...
    for _, e in ipairs(entries) do
        ...
        chip.Parent = row
        do
            local c = Instance.new("UICorner"); c.CornerRadius = UDim.new(0, 4); c.Parent = chip
...
-- OP auto-save: persist on leave / periodic / bind close
task.spawn(function()
    pcall(function()
        if game and game.BindToClose then
            game:BindToClose(function()
                pcall(function() if saveCherryConfig then saveCherryConfig(true) end end)
            end)
        end
    end)
    pcall(function()
...
M.uiColorName = (M.getUiColorEntry and M.getUiColorEntry(M.uiColorName or "Black").name) or "Ocean"
pcall(applyAccentFromTheme)
pcall(saveCherryConfig)
...
M._buildingGui = false
M.menuOpen = true
pcall(function() CAPO_emergencyToast("Capo X Nighthub V3 booting UI...") end)
local function _forceBuild()
    ...
    if not ok then
        warn("[Capo X Nighthub V3] buildGui error: " .. tostring(err))
        pcall(function() CAPO_emergencyToast("Capo X Nighthub V3 UI error — check F9") end)
    end
...
task.delay(1.5, function()
    if not (M.gui and M.gui.Parent and M.mainFrame and M.mainFrame.Parent) then
        warn("[Capo X Nighthub V3] UI still missing — hard rebuild")
        _forceBuild()
    end
end)
task.delay(3.0, function()
    if not (M.gui and M.gui.Parent and M.mainFrame and M.mainFrame.Parent) then
        warn("[Capo X Nighthub V3] UI still missing — final rebuild")
        _forceBuild()
    end
end)
pcall(function() if M.applyBlackSky then M.applyBlackSky() end end)
...
if M.antiBatEnabled then pcall(function() M.startAntiBat() end) end
if M.holdJumpEnabled then pcall(function() M.startHoldJump() end) end
if M.antiDropEnabled then pcall(function() M.startVelocityAntiDrop() end) end
...
player.CharacterAdded:Connect(function(char)
    ...
    if M.hardHitEnabled then task.defer(function() M.hideHardHitRing() end) end
    M.setupRagdollTriggers()
...
task.spawn(function()
    task.wait(2)
    if not (M.gui and M.gui.Parent) then
        warn("[Capo X Nighthub V3] UI missing — forcing main UI")
        M._introUIReady = true
        M._buildingGui = false
        pcall(function()
...
task.defer(function()
    task.wait(0.6)
    if false and M.vynxHornsEnabled and M.setCapoHorns then
        pcall(function() M.setCapoHorns(true) end)
    end
    local char = player.Character
    if char then
        pcall(function() M.applyCharterToChar(char) end)
...
function M.destroyBypassPanel()
    pcall(function()
        ...
        if pg then
            local o = pg:FindFirstChild("CapoBypass")
            if o then o:Destroy() end
        end
        pcall(function()
            local o2 = game:GetService("CoreGui"):FindFirstChild("CapoBypass")
            if o2 then o2:Destroy() end
        end)
        if typeof(gethui) == "function" then
            local h = gethui()
            if h then
                local o3 = h:FindFirstChild("CapoBypass")
...
function M.startPingAlert()
    ...
    strip.BackgroundColor3 = Color3.fromRGB(180, 180, 190)
    ...
    pingLbl.TextColor3 = Color3.fromRGB(255, 230, 160)
...
-- Restore / protect game audio (some optimizers & remote loads mute SFX)
function M.restoreGameAudio()
    pcall(function()
        local SS = game:GetService("SoundService")
        pcall(function() SS.RespectFilteringEnabled = SS.RespectFilteringEnabled end)
        -- unmute common groups if present
        for _, child in ipairs(SS:GetDescendants()) do
            if child:IsA("SoundGroup") then
                pcall(function()
                    if child.Volume ~= nil and child.Volume <= 0 then
                        child.Volume = 1
                    end
                end)
            elseif child:IsA("Sound") then
                pcall(function()
                    if child.Volume ~= nil and child.Volume <= 0.001 and child:GetAttribute("CapoMuted") ~= true then
                        -- do not force play; only un-zero accidental mutes on groups
                    end
                end)
            end
        end
        pcall(function()
            ...
            if ugs and ugs.MasterVolume ~= nil and ugs.MasterVolume <= 0 then
                ugs.MasterVolume = 1
            end
        end)
        -- re-enable workspace/character sounds that were disabled (not destroyed)
        local function unsilence(root)
            if not root then return end
            for _, s in ipairs(root:GetDescendants()) do
                if s:IsA("Sound") then
                    pcall(function()
                        if s.Volume == 0 and s:GetAttribute("CapoKeepMuted") ~= true then
                            -- leave intentionally silent SFX alone if named mute
                            local n = string.lower(tostring(s.Name or ""))
                            if not (n:find("mute") or n:find("silent")) then
                                if s:GetAttribute("CapoOrigVol") then
                                    s.Volume = tonumber(s:GetAttribute("CapoOrigVol")) or 0.5
                                end
                            end
                        end
                    end)
        ...
        pcall(function() unsilence(workspace) end)
        pcall(function() unsilence(player.Character) end)
    end)
end

function M.applyBlackSky()
    local ok, err = pcall(function()
        local Lighting = game:GetService("Lighting")
        -- Soft luxury gold / diamond wash — keeps map sky
        Lighting.OutdoorAmbient = Color3.fromRGB(200, 185, 150)
        Lighting.Ambient = Color3.fromRGB(160, 145, 120)
        Lighting.ColorShift_Top = Color3.fromRGB(255, 230, 170)
        Lighting.ColorShift_Bottom = Color3.fromRGB(140, 120, 80)
        pcall(function() Lighting.EnvironmentDiffuseScale = 0.5 end)
        pcall(function() Lighting.EnvironmentSpecularScale = 0.35 end)

        local atmo = Lighting:FindFirstChild("CapoLuxuryAtmo")
        if not atmo then
            atmo = Lighting:FindFirstChildOfClass("Atmosphere")
            if not atmo then
                atmo = Instance.new("Atmosphere")
                atmo.Name = "CapoLuxuryAtmo"
        ...
        pcall(function()
            atmo.Color = Color3.fromRGB(230, 210, 170)
            atmo.Decay = Color3.fromRGB(160, 130, 80)
            atmo.Density = 0.10
            atmo.Offset = 0.08
            atmo.Glare = 0.12
            atmo.Haze = 0.45
        ...
        local cc = Lighting:FindFirstChild("CapoLuxuryCC")
        if not cc then
            cc = Instance.new("ColorCorrectionEffect")
            cc.Name = "CapoLuxuryCC"
            cc.Parent = Lighting
        end
        pcall(function()
            cc.Enabled = true
        ...
        -- Soft gold/diamond luxury wash, original sky kept
    end)
    if not ok then
        warn("[Capo X Nighthub V3] applyBlackSky failed: " .. tostring(err))
    end
end

print("Capo X Nighthub V3 (Luxury Gold) loaded successfully!")
return M
